<header>
        <div id="logo">
            <center>
                <a href="./?controller=index" style="font-size: 45px; text-decoration: none; color: white;"><p>Logo</p></a>
            </center>
        </div>
        <div class="search">
            <center>
                <div class="mainbox">
                    <div class="iconContainer">
                        <svg viewBox="0 0 512 512" height="1em" xmlns="http://www.w3.org/2000/svg" class="search_icon">
                            <path d="M416 208c0 45.9-14.9 88.3-40 122.7L502.6 457.4c12.5 12.5 12.5 32.8 0 45.3s-32.8 12.5-45.3 0L330.7 376c-34.4 25.2-76.8 40-122.7 40C93.1 416 0 322.9 0 208S93.1 0 208 0S416 93.1 416 208zM208 352a144 144 0 1 0 0-288 144 144 0 1 0 0 288z">
                            </path>
                        </svg>
                    </div>
                    <input class="search_input" placeholder="search" type="text">
                </div>
            </center>
        </div>
        <div id="bell">
            <a href="#"><i class="fa fa-bell" style="font-size:28px;color:white"></i></a>
        </div>
        <div id="icon-user">
            <a href="#" id="icon-user1">
                <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="currentColor" class="bi bi-person-circle" viewBox="0 0 16 16" style="color: white;">
                    <path d="M11 6a3 3 0 1 1-6 0 3 3 0 0 1 6 0z" />
                    <path fill-rule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1z" />
                </svg>
            </a>
        </div>
    </header>
    <div style="background-color: antiquewhite; width: 100%; height: 60px;">
        <nav>
            <div>
                <a href="">Việc quan trọng cần làm</a>
            </div>
            <div>
                <a href="">Mục tiêu tháng</a>
            </div>
            <div>
                <a href="">Một tháng nhìn lại</a>
            </div>
            <div>
                <a href="./?controller=dieuBietOn&action=index">Điều biết ơn mỗi ngày</a>
            </div>
            <div>
                <a href="">Biểu đồ cảm xúc của tôi</a>
            </div>
            <div>
                <a href="./taothoiquen.php">Những thói quen tôi muốn có</a>
            </div>
        </nav>
    </div>